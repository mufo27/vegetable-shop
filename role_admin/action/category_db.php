...
<head>
    ...
    <title>กำละงดำเนินการหมวดหมู่</title>
    <!-- sweetalert2 -->
...
<body>
        ...
        if ($select_check->rowCount() > 0) {
            ...
            try {
                ...
                if ($_FILES['img']['tmp_name'] == "") {
                    ...
                    $extension      =   end($temp);
                    $newfilename    =   'ty_' . round(microtime(true)) . '.' . end($temp);
        ...
        if ($select_check->rowCount() > 1) {
            ...
            try {
                ...
                if ($_FILES['img']['tmp_name'] == "") {
                    ...
                    $size_file      =   $_FILES['img']['size'];
                    $extension      =   end($temp);
                    $newfilename    =   'ct_' . round(microtime(true)) . '.' . end($temp);
    ...
    if (isset($_POST['btn_del'])) {
        ...
        try {
            ...
            $select_img = $conn->prepare("SELECT img FROM category WHERE id=?");
            ...
            $delete_tb2 = $conn->prepare("DELETE FROM category WHERE id=?");
            $delete_tb2->bindParam(1, $id);
            $delete_tb2->execute();

            if ($delete_tb1 && $delete_tb2) {
